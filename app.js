...
function renderizarVendas() {
    ...
    vendasGlobais.forEach(venda => {
        ...
        if(venda.Metodo_Pagamento) metodoTxt = typeof venda.Metodo_Pagamento === 'object' ? (venda.Metodo_Pagamento.value || "Dinheiro") : venda.Metodo_Pagamento;
...
function atualizarSelects() {
    ...
    if(!sV || !sE) return;
    sV.innerHTML = '<option value="">Selecione um produto...</option>'; sE.innerHTML = '<option value="">Selecione um produto...</option>';
    ...
    produtosGlobais.forEach(p => {
        ...
        let optVenda = new Option(`${p.Produto} ${p.Modelo} - ${p.Cor} (${p.Tamanho}) | Estoque: ${estoqueReal} un.`, p.id);
        
        if (estoqueReal <= 0) { optVenda.text = `🚫 ESGOTADO - ${p.Produto} ${p.Modelo} - ${p.Cor} (${p.Tamanho})`; optVenda.disabled = true; }
        sV.add(optVenda);
        sE.add(new Option(`${p.Produto} ${p.Modelo} - ${p.Cor} (${p.Tamanho}) | Estoque: ${(parseInt(p.Estoque) || 0)}`, p.id));
    });
}
...
function adicionarAoCarrinho() {
    ...
    if (qtd > estoqueDisponivel) return mostrarNotificacao(`Você só tem mais ${estoqueDisponivel} unidade(s) disponíveis!`, "erro");

    carrinhoVendas.push({ id: p.id, nomeCompleto: `${p.Produto} ${p.Modelo} - ${p.Cor} (${p.Tamanho})`, qtd: qtd, preco: limparNumero(p.Preco_Venda), custo: limparNumero(p.Custo), estoqueOriginal: parseInt(p.Estoque) || 0 });
    document.getElementById("quantidadeVenda").value = 1; document.getElementById("produtoVenda").value = "";
    renderizarCarrinho(); atualizarSelects();
}

function removerDoCarrinho(index) { carrinhoVendas.splice(index, 1); renderizarCarrinho(); atualizarSelects(); }

function renderizarCarrinho() {
    const area = document.getElementById("areaCarrinho"), lista = document.getElementById("listaCarrinho");
    if (carrinhoVendas.length === 0) { area.style.display = "none"; lista.innerHTML = ""; atualizarResumoCarrinho(); return; }
    area.style.display = "block"; lista.innerHTML = "";
    
    carrinhoVendas.forEach((item, index) => {
        lista.innerHTML += `<div class="carrinho-item"><div class="carrinho-info"><strong>${item.qtd}x ${item.nomeCompleto}</strong><span class="carrinho-preco">${formatarMoeda(item.preco * item.qtd)}</span></div><button class="btn-remover" onclick="removerDoCarrinho(${index})">❌</button></div>`;
    });
...
async function salvarEdicaoVenda() {
    const id = document.getElementById("editVendaID").value, qA = parseInt(document.getElementById("editVendaQtdAntiga").value), nQ = parseInt(document.getElementById("editVendaQuantidade").value), pr = limparNumero(document.getElementById("editVendaPreco").value), cus = limparNumero(document.getElementById("editVendaCusto").value), desc = limparNumero(document.getElementById("editVendaDesconto").value), tP = limparNumero(document.getElementById("editVendaTaxa").value), codPedido = document.getElementById("editVendaCodigo").value;
    const fR = Number(((pr * nQ) - desc).toFixed(2)), vT = Number((fR * (tP / 100)).toFixed(2)), luc = Number((fR - vT - (cus * nQ)).toFixed(2));
    const payload = { id, produtoID: document.getElementById("editVendaProdutoID").value, diferencaEstoque: qA - nQ, novaVenda: { Quantidade: nQ, cliente: document.getElementById("editVendaCliente").value, Data: document.getElementById("editVendaData").value, Desconto: desc, Metodo_Pagamento: document.getElementById("editVendaMetodo").value, Taxa_Maquininha: vT, Faturamento: fR, Lucro_Venda: luc, Codigo_Pedido: codPedido } };
    try {
        const res = await fetch('/api/editar-venda', { method: 'PATCH', headers: { 'Content-Type': 'application/json', 'Authorization': SENHA_ACESSO }, body: JSON.stringify(payload) });
        if (res.ok) { mostrarNotificacao("Atualizado!"); fecharModalVenda(); carregarDados(); } else mostrarNotificacao("Erro ao atualizar venda.", "erro");
    } catch (e) { mostrarNotificacao("Erro de conexão", "erro"); }
}

function abrirModalEdicaoVenda(id) {
    const v = vendasGlobais.find(x => x.id === id);
    document.getElementById("editVendaID").value = v.id; document.getElementById("editVendaProdutoID").value = v.Produtos[0].id; document.getElementById("editVendaQtdAntiga").value = v.Quantidade; document.getElementById("editVendaPreco").value = limparNumero(v.Preco_Venda); document.getElementById("editVendaCusto").value = limparNumero(v.Custo_Produto); document.getElementById("editVendaCodigo").value = v.Codigo_Pedido || "";
    let nomeProduto = v.Produtos[0].value; const prodEncontrado = produtosGlobais.find(p => p.id === v.Produtos[0].id);
    if(prodEncontrado) nomeProduto = `${prodEncontrado.Produto} ${prodEncontrado.Modelo} - ${prodEncontrado.Cor} (${prodEncontrado.Tamanho})`;
    document.getElementById("editVendaProdutoNome").value = nomeProduto; document.getElementById("editVendaCliente").value = v.cliente || ""; document.getElementById("editVendaData").value = v.Data?.split('T')[0] || ""; document.getElementById("editVendaQuantidade").value = v.Quantidade; document.getElementById("editVendaDesconto").value = limparNumero(v.Desconto);
    let metodoTxt = "Dinheiro"; if (v.Metodo_Pagamento) metodoTxt = typeof v.Metodo_Pagamento === 'object' ? (v.Metodo_Pagamento.value || "Dinheiro") : v.Metodo_Pagamento;
    document.getElementById("editVendaMetodo").value = metodoTxt;
    let fR = limparNumero(v.Faturamento), vT = limparNumero(v.Taxa_Maquininha); document.getElementById("editVendaTaxa").value = fR > 0 ? (vT / fR * 100).toFixed(2) : 0;
...
async function excluirVenda(id) {
    ...
    const v = vendasGlobais.find(x => x.id === id);
    try {
        const res = await fetch(`/api/excluir-venda?id=${id}&produtoID=${v.Produtos[0].id}&quantidadeRetorno=${v.Quantidade}`, { method: 'DELETE', headers: { 'Authorization': SENHA_ACESSO } });
    ...
    try {
        ...
        vendasGlobais.forEach(v => {
            ...
            if (v.Produtos && v.Produtos.length > 0) {
                ...
                if (produtoNoBanco) { prodNome = `${produtoNoBanco.Produto} ${produtoNoBanco.Modelo} - ${produtoNoBanco.Cor} (${produtoNoBanco.Tamanho})`; corDoProduto = produtoNoBanco.Cor ? produtoNoBanco.Cor.toLowerCase() : ""; } else { prodNome = v.Produtos[0].value; }
            }
        ...
        let coresDasBarras = produtosOrdenados.map(p => { let corText = p[1].cor; if (corText.includes("amarel")) return "#ffd54f"; if (corText.includes("azul")) return "#64b5f6"; if (corText.includes("pret")) return "#616161"; if (corText.includes("branc")) return "#e0e0e0"; if (corText.includes("verd")) return "#81c784"; if (corText.includes("vermelh")) return "#e57373"; return "#ffa726"; });
        graficoTopProdutos = new Chart(document.getElementById('graficoBarras'), { type: 'bar', data: { labels: produtosOrdenados.map(p => { let nome = p[0]; return nome.length > 50 ? nome.substring(0, 50) + '...' : nome; }), datasets: [{ label: 'Unidades Vendidas', data: produtosOrdenados.map(p => p[1].qtd), backgroundColor: coresDasBarras, borderRadius: 4 }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false } });

        if(graficoPagamentos) graficoPagamentos.destroy();
        graficoPagamentos = new Chart(document.getElementById('graficoPizza'), { type: 'doughnut', data: { labels: Object.keys(fatPorPagamento), datasets: [{ data: Object.values(fatPorPagamento), backgroundColor: ['#5c6bc0', '#66bb6a', '#ffa726', '#ef5350'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false } });
